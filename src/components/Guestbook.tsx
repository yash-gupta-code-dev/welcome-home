...
export default function Guestbook({ onClose }: GuestbookProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col text-slate-100 font-sans" id="guestbookComponent">
      ...
      {/* parchment header */}
      <div className="flex flex-col md:flex-row gap-4 md:items-center justify-between pb-4 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Leave a signature note on the left, and view all stored wishes together on the right!
            </p>
          </div>
        </div>
      ...
      {/* Main Content Pane Grid split into 2 Columns (Form & Messages side-by-side on tablet/desktop) */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mt-4 flex-grow overflow-y-auto md:overflow-hidden pr-1">
            ...
            <form onSubmit={handleAddEntry} className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="relative">
                  <input
                    type="text"
                    placeholder="Your Name (e.g., Pragati, Hardik)"
                    value={newName}
        ...
        {/* Right Column (List of entries, scrollable & highly visible) */}
        <div className="md:col-span-7 flex flex-col min-h-0 bg-slate-950/40 border border-slate-800/80 rounded-2xl p-4 shadow-xl" id="storedWishesSection">
          <div className="flex flex-wrap justify-between items-center gap-2 mb-3 border-b border-slate-800/80 pb-2.5">
            <div className="flex items-center gap-2">
              <span className="text-base">📜</span>
              <h4 className="text-sm md:text-base font-bold text-sky-300 flex items-center gap-1.5">
                <span>Stored Birthday Wishes</span>
                <span className="text-xs bg-sky-500/20 text-sky-300 px-2 py-0.5 rounded-full border border-sky-500/30">
                  {entries.length} Saved
                </span>
              </h4>
            </div>
            <span className="px-2.5 py-0.5 bg-amber-500/10 border border-amber-500/30 text-amber-300 text-[11px] font-mono rounded-full">
              🎂 August 18th, 2026
            </span>
          </div>

          <div className="flex-grow overflow-y-auto pr-1 space-y-3.5 max-h-[55vh] md:max-h-[62vh]">
